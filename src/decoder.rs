...
pub enum Instruction {
    IInstruction(String, u8, u8, u8, u8),
    RInstruction(u8, u8, u8, u8, u8, u8),
    SInstruction(String, u8, u8, u8, u8),
    JInstruction(String, u8, u8),
    BInstruction(String, u8, u8, u8, u8),
    UInstruction(String, u8, u8),
    OtherInstruction,
}
...
fn decode_i_instruction(inst: [MemoryValue; 4]) -> Instruction {
    // let imm: i16 = ((inst[3] as i16) << 4) + ((inst[2] as i16) >> 4);
    ...
    let op = inst[0] & 127;
    Instruction::IInstruction(imm_string, rs1, funct3, rd, op)
}
...
fn decode_s_instruction(inst: [MemoryValue; 4]) -> Instruction {
    // let imm: i16 =
    //     ((inst[3] as i16 >> 1) << 5) + ((inst[1] as i16 & 15) << 1) + (inst[0] as i16 >> 7);
    ...
    let funct3 = (inst[1] & 127) >> 4;
    let op = inst[0] & 127;
    Instruction::SInstruction(imm_string, rs2, rs1, funct3, op)
}
...
fn decode_j_instruction(inst: [MemoryValue; 4]) -> Instruction {
    ...
    let rd = ((inst[1] & 15) << 1) + (inst[0] >> 7);
    let op = inst[0] & 127;
    Instruction::JInstruction(imm_string, rd, op)
}
...
fn decode_b_instruction(inst: [MemoryValue; 4]) -> Instruction {
    ...
    let funct3 = (inst[1] & 127) >> 4;
    let op = inst[0] & 127;
    Instruction::BInstruction(imm_string, rs2, rs1, funct3, op)
}
...
fn decode_u_instruction(inst: [MemoryValue; 4]) -> Instruction {
    ...
    let rd = ((inst[1] & 15) << 1) + (inst[0] >> 7);
    let op = inst[0] & 127;
    Instruction::UInstruction(imm_string, rd, op)
}
